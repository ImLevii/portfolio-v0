...
export async function POST(req: Request) {
    ...
    if (event.type === "charge:confirmed" || event.type === "charge:resolved") {
        ...
        try {
            ...
            // Create Order
            // Calculate total from payment details or trust metadata? 
            // Better to trust the database items but for simplicity of webhook, and since paid, we can just record it.
            // However, consistent with other flows, we should probably fetch items.
                ...
                data: {
                    ...
                    items: {
                        create: items.map((item) => ({
                            productId: item.id,
                            price: item.price // Note: This might differ from actual paid amount if there was a discount, but good for record
                        }))
                    }
                }
            })
